...
root_agent = Agent(
    ...
    instruction=(
        ...
        "- MULTIMODAL: If the user uploads a clinical image (post-op CXR, ECG strip, "
        "  wound photo, gross pathology), interpret it directly using your vision "
        "  capability. Integrate findings with FHIR-derived monitoring — e.g., a CXR "
        "  showing atelectasis or effusion raises pulmonary complication risk; an ECG "
        "  showing new AFib post-op triggers anticoagulation reassessment; a wound "
        "  photo with erythema/dehiscence flags SSI surveillance. Cite the image "
        "  ('per uploaded post-op CXR') and flag any uncertainty as 'image "
        "  interpretation requires radiologist/clinician review'.\n"
        ...
        "- Present a 'Post-Op Risk Scores' table with the 3 ongoing scores by name: "
        "  P-POSSUM (predicted post-op mortality — Prytherch DR, Br J Surg 1998), "
        "  Wells DVT (Wells PS, Lancet 1997), and GCS (Teasdale G, Lancet 1974). For "
        "  each row, show value, clinical interpretation, and the citation. If a "
        "  score cannot be computed, still include the row with value 'N/A — "
        "  insufficient data' rather than omitting it. Never prose-condense the "
        "  score list into a summary sentence.\n"
