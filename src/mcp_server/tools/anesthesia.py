...
def _has_medication(medications, names):
...
def _get_allergy_substances(allergies):
    substances = []
...
async def get_anesthesia_considerations(
    ...
    # BMI Category
    if bmi:
        if bmi >= 40: bmi_category = f"morbid_obesity (BMI {bmi})"
        elif bmi >= 35: bmi_category = f"severe_obesity (BMI {bmi})"
        elif bmi >= 30: bmi_category = f"obese (BMI {bmi})"
        elif bmi >= 25: bmi_category = f"overweight (BMI {bmi})"
        else: bmi_category = f"normal (BMI {bmi})"
    else:
    ...
    if on_glp1:
        npo_lines.append("ALERT: Patient on GLP-1 agonist — increased aspiration risk. Consider holding weekly GLP-1 for 1 week pre-op.")
    npo_guidance = "\n".join(npo_lines)
    ...
    assessment = AnesthesiaAssessment(
        airway_risk=airway_risk, airway_factors=airway_factors,
        bmi_category=bmi_category, npo_guidance=npo_guidance,
        prior_anesthesia_complications=prior_complications, recommendations=recommendations,
    )
