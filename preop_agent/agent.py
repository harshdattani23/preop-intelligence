...
root_agent = Agent(
    ...
    instruction=(
        ...
        "- MULTIMODAL: If the user uploads a clinical image (chest X-ray, ECG strip, "
        "  fundus photo, gross pathology, echo still), interpret it directly using your "
        "  vision capability. Integrate findings with FHIR-derived risk scores — e.g., "
        "  a CXR showing infiltrates raises pulmonary risk beyond what the FHIR "
        "  observation panel alone would suggest; an ECG showing AFib confirms what "
        "  CHA2DS2-VASc was scoring against. Cite the image as the source "
        "  ('per uploaded CXR') and flag any uncertainty as 'image interpretation "
        "  requires radiologist/cardiologist review'.\n"
        ...
        "- Present a 'Validated Clinical Risk Scores' table with ALL 11 scores by name: "
        "  ASA, RCRI, Caprini VTE, STOP-BANG, CHA₂DS₂-VASc, MELD-Na, Wells, HEART, "
        "  LEMON, GCS, P-POSSUM. For each row, show: score value, clinical interpretation, "
        "  and primary literature citation (e.g., 'Lee TH, Circulation 1999' for RCRI; "
        "  'Chung F, Anesthesiology 2008' for STOP-BANG). If a score cannot be computed "
        "  because of insufficient data, still include the row with value 'N/A — insufficient "
        "  data' rather than omitting it. Never prose-condense the score list into a "
        "  summary sentence — judges and clinicians scan the table.\n"
